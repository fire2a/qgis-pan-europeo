...
class MarraquetaDialog(QDialog):
    ...
    def populate(self, raster_map_layers):
        ...
        self.rows = []
        i = 0
        for layer in raster_map_layers:
            # name
    ...
    def reject(self):
        qprint("MarraquetaDialog.reject")
        self.destroy()

    def reset(self):
        qprint("MarraquetaDialog.reset")
        self.populate(get_raster_map_layers())
        # parent = self.parent()
        # self.destroy()
...
def get_raster_map_layers(extent=None):
    qprint("Marraqueta.get_raster_map_layers")
    ...
    for layer_id, layer in QgsProject.instance().mapLayers().items():
        if (layer.type() == QgsMapLayer.RasterLayer) and (Path(layer.publicSource()).is_file()):
            ...
            ret += [info]
            qprint(f"{info['name']=} {info['Minimum']=} {info['Maximum']=}")
    return ret
