...
from qgis.core import (QgsProcessingException, QgsProcessingParameterDefinition, QgsProcessingParameterEnum,
...
class ProcessingGdalCalcNormAlgorithm(GdalAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        self.addParameter(QgsProcessingParameterRasterLayer(self.INPUT_A, self.tr("Input layer"), optional=False))
        # self.addParameter(
        #     QgsProcessingParameterBand(
        #         self.BAND_A,
        #         self.tr("Number of raster band"),
        ...
        min_param = QgsProcessingParameterNumber(
            self.MIN,
            self.tr(
                "<b>Minimun</b> value for minmax/maxmin (else the value is calculated from the WHOLE input raster)."
            ),
            type=QgsProcessingParameterNumber.Type.Double,
            defaultValue=None,
            optional=True,
        )
        min_param.setFlags(min_param.flags() | QgsProcessingParameterDefinition.Flag.FlagAdvanced)
        self.addParameter(min_param)
        max_param = QgsProcessingParameterNumber(
            self.MAX,
            self.tr(
                "<b>Maximum</b> value for minmax/maxmin (else the value is calculated from the WHOLE input raster)."
            ),
            type=QgsProcessingParameterNumber.Type.Double,
            defaultValue=None,
            optional=True,
        )
        max_param.setFlags(max_param.flags() | QgsProcessingParameterDefinition.Flag.FlagAdvanced)
    ...
    def displayName(self):
        return self.tr("Raster Normalizator")
    ...
    def getConsoleCommands(self, parameters, context, feedback, executing=True):
        ...
        if self.MIN in parameters and parameters[self.MIN] is not None:
            min_param = self.parameterAsDouble(parameters, self.MIN, context)
            arguments.append(f"--minimum {min_param}")

        if self.MAX in parameters and parameters[self.MAX] is not None:
            max_param = self.parameterAsDouble(parameters, self.MAX, context)
            arguments.append(f"--maximum {max_param}")
        ...
        arguments.append('"' + layer_details.connection_string + '"')
        # if self.parameterAsString(parameters, self.BAND_A, context):
        #     arguments.append(f"--A_band {self.parameterAsString(parameters, self.BAND_A, context)}")
        ...
        if self.PARAMS in parameters and parameters[self.PARAMS] not in (None, ""):
            arguments.append("--")
